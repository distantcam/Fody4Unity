...
namespace MadProps.AppArgs
{
    public static class AppArgs
    {
        ...
        static IEnumerable<ArgProperty> PropertiesOf<T>() => from p in typeof(T).GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.SetProperty)
    ...
    {
        public static T Attribute<T>(this PropertyInfo p) => p.GetCustomAttributes(typeof(T), true).Cast<T>().FirstOrDefault();
    }
}